...
class ParallelDeepseekV2Model(DeepseekV2Model):
...
class ParallelDeepseekV2ForCausalLM(DeepseekV2ForCausalLM):
	def __init__(self, config, n_cuda = 2):
		super(DeepseekV2ForCausalLM, self).__init__(config)
		self.model = ParallelDeepseekV2Model(config, n_cuda = n_cuda)

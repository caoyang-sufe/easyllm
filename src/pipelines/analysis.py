...
# Rank analysis of layer output derived from one-time-forward generation
...
def rank_analysis_of_forward_hook(
	hook_data = None,
	hook_data_path = None,
	num_hooked_modules = None,
	ncols = 4,
	figure_size = 5,
	thresholds = [.1, .05, .01],
	device = "cpu",
	...
	assert hook_data is not None or hook_data_path is not None
	if hook_data is None:
		hook_data = torch.load(hook_data_path, weights_only=False, map_location=torch.device(device))
	if num_hooked_modules is None:
		num_hooked_modules = len(hook_data)
	nrows = num_hooked_modules // ncols + (num_hooked_modules % ncols != 0)
	fig, axes = plt.subplots(
		nrows = nrows,
		ncols = ncols,
		figsize = (figure_size * 1.2 * ncols, figure_size * nrows),
	...
	if save_dir is not None:
		plt.savefig(os.path.join(save_dir, f"layer_outputs_singular_values_{time_string}.png"))
	else:
		plt.show()
	plt.close()
	...
	plt.ylabel("Rank")
	if save_dir is not None:
		plt.savefig(os.path.join(save_dir, f"rank_{time_string}.png"))
	else:
		plt.show()
	plt.close()
